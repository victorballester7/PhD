...
\def\captionstabilityplot{Classification of the topological behavior of points downstream of the gap.}
...
\begin{frame}{Setup}
	...
	\begin{itemize}
		...
		\item $\delta^*$ measured at the upstream edge of the gap in the smooth flat plate.
	\end{itemize}
...
\begin{frame}{Computational cost}
	...
	\begin{itemize}
		\item Mesh: $\sim 10\, 000$ elements
		\item Polynomial order: $(7,6)$
		\item Time step: from  $5 \times 10^{-4}$ (on transient conditions) \\\hspace{1.9cm}to $5 \times 10^{-3}$ (on steady conditions)
		\item HPC: 1 node with 256 CPUs
	\end{itemize}
...
\begin{frame}{$\exp{N}$-method results}
	\begin{figure}
		...
		\captionof{figure}{Interpolated $\Delta N = N - N_\text{ref}$ in the equilibra region.}
	\end{figure}
	\vspace{0.42cm}
\end{frame}
\begin{frame}{$\exp{N}$-method results}
