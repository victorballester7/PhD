...
void assembound(int np, int P, int eln, int Nel, double *z, double *w,
                double *phi1, double *phi2, double **M, double *f, double *p,
                double *Jac, double *u_D, double *bc);
void rhbc(int np, int P, int eln, int Nel, double *phi, double *z, double *u_D,
          double *bc);
...
void assem(int np, int P, int eln, double *z, double *w, double *phi1,
           double *phi2, double **M, double *f, double *p, double *Jac);
void basis(int np, int P, int i, double *z, double *phi);
double **dmatrix(int Mdim);
void sol(int np, int P, int Nel, double *z, double *phi, double *f_G,
         double *u_H, double *u_D, double *u_DG, double *bc);
void GlobAssem(int np, int P, int Nel, int Mdim, double *z, double *w,
               double *phi1, double *phi2, double **M, double *f, double *p,
               double *Jac, double *u_D, double *bc, double **M_G, double *f_G,
               double *x, double *x_G, double *bound, int **map);

extern "C" {
extern void dgetrf_(int *, int *, double(*), int *, int[], int *);
}
extern "C" {
extern void dgetrs_(unsigned char *, int *, int *, double(*), int *, int[],
                    double[], int *, int *);
}
...
int main() {
  ...
  double *x, *x_G, *z, *w, *p, *f, *f_G, *phi1, *phi2, *u_H, *u_D, *u_DG, *bc,
      **M, **M_G, sum = 0, *Jac, *bound;
  ...
  GlobAssem(np, P, Nel, Mdim, z, w, phi1, phi2, M, f, p, Jac, u_D, bc, M_G, f_G,
            x, x_G, bound, map);

  // print non-zero elements of Mg
  for (int i = 0; i < Mdim; i++) {
    for (int j = 0; j < Mdim; j++) {
      if (abs(M_G[i][j]) > 1e-10) {
	cout << M_G[i][j] << endl;
      }
    }
  }
...
void sol(int np, int P, int Nel, double *z, double *phi, double *f_G,
  ...
  for (int eln = 0; eln < Nel; eln++) {
    if (eln == 0) {
      for (int i = 1; i < P + 1; i++) {
        ...
        for (int j = 0; j < np; j++) {
          u_H[j + (eln * (np - 1))] =
              u_H[j + (eln * (np - 1))] + f_G[i + (eln * P) - 1] * phi[j];
        }
      }
    } else if (eln == Nel - 1) {
      for (int i = 0; i < P; i++) {
  ...
  rhbc(np, P, eln, Nel, phi, z, u_D, bc);
  for (int i = 0; i < np; i++) {
    u_DG[i + (Nel * np - (Nel - 1)) - np] =
        u_DG[i + (Nel * np - (Nel - 1)) - np] + u_D[i];
  }
}
...
void GlobAssem(int np, int P, int Nel, int Mdim, double *z, double *w,
  ...
  for (int eln = 0; eln < Nel; eln++) {
    ...
    if (eln == 0) {
      ...
      for (int a = 0; a < P; a++) {
        for (int b = 0; b < P; b++) {
          M_G[map[eln][a]][map[eln][b]] =
              M_G[map[eln][a]][map[eln][b]] + M[a][b];
        }
