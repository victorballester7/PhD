...
void buildfe(int Q, int P, double *f, double *fquadPoints, double *polyMatrix,
             double *z, double *w, double a, double b);
void buildfg(int Nel, int Q, int P, int dim, double *fg, int *mapArray,
             double *fquadPoints, double *polyMatrix, double *z, double *w,
             double A, double B);
void solveSystem(int dim, double *Mg, double *fg);
void getUapprox(int Nel, int Q, int P, double *u_approx, double *fg,
...
int main() {
  ...
  buildfg(Nel, Q, P, dim, fg, mapArray, fquadPoints, polyMatrix, z, w, A, B);
  solveSystem(dim, Mg, fg);
  getUapprox(Nel, Q, P, u_approx, fg, mapArray, polyMatrix);
  ...
  for (int e = 0; e < Nel; e++) {
    for (int i = 0; i < Q; i++) {
      if (i == Q - 1 && e != Nel - 1)
        continue;
      func(Q, z, fquadPoints, A + e * (B - A) / Nel, A + (e + 1) * (B - A) / Nel);
      cout << "f(xi_" << e << "," << i << ") = " << fquadPoints[i] << "\t" << u_approx[e * (Q - 1) + i]
...
void buildfe(int Q, int P, double *f, double *fquadPoints, double *polyMatrix,
  ...
  for (int p = 0; p < P + 1; p++)
    f[p] = gaussIntegration(Q, z, w, fquadPoints, polyMatrix + p * Q, a, b);
}
...
void buildfg(int Nel, int Q, int P, int dim, double *fg, int *mapArray,
  ...
  for (int e = 0; e < Nel; e++) {
    buildfe(Q, P, fe, fquadPoints, polyMatrix, z, w, A + e * el_length,
            A + (e + 1) * el_length);
    for (int p = 0; p < P + 1; p++) {
      fg[map(e, p)] += fe[p];
    }
