...
void buildM(int Q, int P, double *M, double *polyMatrix, double *z, double *w,
            double a, double b);
void buildf(int Q, int P, double *f, double *fquadPoints, double *polyMatrix,
            double *z, double *w, double a, double b);
void solveSystem(int Q, int P, double *M, double *f);
void getUapprox(int Q, int P, double *u_approx, double *f, double *polyMatrix);
...
int main() {
  ...
  buildf(Q, P, f, fquadPoints, polyMatrix, z, w, a, b);
  solveSystem(Q, P, M, f);
  getUapprox(Q, P, u_approx, f, polyMatrix);
...
void getUapprox(int Q, int P, double *u_approx, double *f, double *polyMatrix) {
  for (int i = 0; i < Q; i++) {
    u_approx[i] = 0;
    for (int p = 0; p < P + 1; p++) {
      u_approx[i] += f[p] * polyMatrix[p * Q + i];
    }
  }
}
