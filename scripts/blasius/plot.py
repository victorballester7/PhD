...
def plot(x, y, incNS, ufit, vfit, plot_dns, x_eq_L, path_dns, param):
    ...
    diffV = (
        x * y[2]
        if incNS
        else y[2] * cumulative_trapezoid(y[3], x, initial=0) - y[0] * y[4]
    )
    ...
    for i, (u_apx, v_apx) in enumerate(zip(u_approx, v_approx)):
        ...
        diffUapx = np.gradient(u_apx, x,edge_order=2)
        diffVapx = np.gradient(v_apx, x,edge_order=2)
        ...
        print(f"||u - u_aprox_{i}||_oo / ||u||_oo    = ", errUoo / Unormoo)
        print(f"||v - v_aprox_{i}||_oo / ||v||_oo    = ", errVoo / Vnormoo)
        print(f"||u' - u_aprox_{i}'||_oo / ||u'||_oo = ", errdiffUoo / diffUnormoo)
        print(f"||v' - v_aprox_{i}'||_oo / ||v'||_oo = ", errdiffVoo / diffVnormoo)

        print(
            f"||u - u_aprox_{i}||_C1 / ||u||_C1    = ", (errUoo + errdiffUoo) / UnormC1
        )
        print(
            f"||v - v_aprox_{i}||_C1 / ||v||_C1    = ", (errVoo + errdiffVoo) / VnormC1
        )
...
def plot_comNS(x, y, fits, plot_dns, x_eq_L, path_dns, param):
    ...
    for real_val, approx_val, lab in zip(real, approx, labels):
        print(
            f"||{lab} - {lab}_approx||_oo / ||{lab}||_oo = {la.norm(real_val - approx_val, np.inf) / la.norm(real_val, np.inf)}"
        )
