...
class PlotLimits(BaseModel):
    ...
    def default(cls, branch: Branch, problem: ProblemType) -> "PlotLimits":
        ...
        if branch == Branch.Temporal:
            if problem in {ProblemType.BoundaryLayer, ProblemType.Gap}:
...
class Config(BaseModel):
    ...
    def from_toml(cls, file_path: str) -> "Config":
        ...
        data["general"]["filenameUprofile"] = Path(data["general"]["filenameUprofile"])
        ...
        parsed_data = {
            **data["general"],
            **data["flags"],
            "vars_r": VarsRange(**data["vars"]["vars_r"]),
            "vars_i": VarsRange(**data["vars"]["vars_i"]),
